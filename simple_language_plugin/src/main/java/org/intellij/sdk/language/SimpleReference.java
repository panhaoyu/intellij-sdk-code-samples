...
public final class SimpleReference extends PsiReferenceBase<SimpleTkIdentifierRaw> implements PsiPolyVariantReference {
    ...
    // 构造函数，初始化引用和键值
    public SimpleReference(@NotNull SimpleTkIdentifierRaw element, TextRange textRange) {
    ...
    public ResolveResult @NotNull [] multiResolve(boolean incompleteCode) {
        ...
        final List<SimpleTkIdentifierRaw> identifiers = SimpleUtil.findIdentifiers(project, name); // 查找与键值相关的属性列表
        List<ResolveResult> results = new ArrayList<>(); // 创建解析结果列表
        for (SimpleTkIdentifierRaw identifier : identifiers) { // 遍历找到的属性
    ...
    @Nullable
    @Override
    public SimpleTkIdentifierRaw resolve() {
        ResolveResult[] resolveResults = multiResolve(false);
        return resolveResults.length >= 1 ? (SimpleTkIdentifierRaw) resolveResults[0].getElement() : null;
    }
    ...
    public Object @NotNull [] getVariants() {
        Project project = myElement.getProject();
        List<SimpleTkIdentifierRaw> identifiers = SimpleUtil.findIdentifiers(project); // 获取所有属性
        List<LookupElement> variants = new ArrayList<>(); // 创建自动补全选项列表
        Set<String> seenNames = new HashSet<>(); // 用于跟踪已经添加的名称

        for (final SimpleTkIdentifierRaw identifier : identifiers) { // 遍历所有属性
